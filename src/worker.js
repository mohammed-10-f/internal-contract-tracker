...
await env.DB.prepare("UPDATE records SET status=?,final_approved_at=NULL,timer_end_at=NULL,timer_paused_at=NULL,paused_seconds=COALESCE(paused_seconds,0)+?,stopped_at=NULL,stopped_by=NULL,stage_started_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(next,pauseAdd,id).run();
 ...
 if(p==="/api/manager-stats"&&req.method==="GET"){
  ...
  return json({manager,filters:{region,from,to},total,required:Number(row.required||0),awaiting:Number(row.awaiting||0),completed,overdue:Number(row.overdue||0),returned:Number(row.returned||0),documented:Number(row.documented||0),withdrawn:Number(row.withdrawn||0),stopped:Number(row.stopped||0),delegated:Number(row.delegated||0),receivedByDelegation:Number(row.delegated||0),delegatedOut:0,completionRate:total?Math.round(completed/total*100):0,withinSlaRate:withinSla,avg_duration_seconds:avgDuration,avg_duration_label:durationLabel(avgDuration),median_duration_seconds:median,median_duration_label:durationLabel(median),avg_response_seconds:avgResponse,avg_response_label:durationLabel(avgResponse),recent,statusBreakdown,report_date:reportDate()});
 }
